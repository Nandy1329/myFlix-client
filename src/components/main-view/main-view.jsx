...
export const MainView = () => {
    ...
    const handleUserUpdate = async (updatedUser) => {
        try {
            const response = await fetch(`https://myflixdb1329-efa9ef3dfc08.herokuapp.com/users/${user.Username}`, {
                method: "PUT",
                headers: {
    ...
    const handleUserDelete = async (username) => {
        try {
            const response = await fetch(`https://myflixdb1329-efa9ef3dfc08.herokuapp.com/${username}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` }
            });
            if (!response.ok) {
    ...
    const addFavorite = async (movie) => {
        try {
            const response = await fetch(`https://myflixdb1329-efa9ef3dfc08.herokuapp.com/users/${user.Username}/movies/${movie._id}`, {
                method: "PATCH",
                headers: { Authorization: `Bearer ${token}` }
            });
            if (!response.ok) {
